...
export async function POST(req: NextRequest) {
  try {
    ...
    const systemPrompt = `Ban la tro ly AI ca nhan cua CalSnap.
...
BUA AN HOM NAY:
${todayMeals?.map((m: any) => `- ${m.food_name}: ${m.calories} kcal`).join('\n') || '- Chua co bua nao'}

CA NHAN:
- Muc tieu: ${profile?.goal ?? 'chua set'}
- Can nang: ${profile?.weight_kg ?? '?'}kg -> ${profile?.target_weight_kg ?? '?'}kg
- Streak: ${profile?.journey_streak ?? 0} ngay
${plan ? `- Plan: ${plan.daily_calories} kcal, ${plan.daily_protein_g}g protein` : ''}
